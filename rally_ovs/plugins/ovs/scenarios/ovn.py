...
class OvnScenario(ovnclients.OvnClientMixin, scenario.OvsScenario):
    ...
    def _create_lports(self, lswitch, lport_create_args = [], lport_amount=1, lport_ip_shift = 0):
        ...
        if network_cidr:
            ...
            ip_addrs = netaddr.IPNetwork(network_cidr.ip + lport_ip_shift).iter_hosts()
    ...
    def _create_acl(self, lswitch, lports, acl_create_args, acls_per_port):
        ...
        extra_match_val = acl_create_args.get("match", "")
        ...
        ovn_nbctl.enable_batch_mode()
        for lport in lports:
            match = '%s == "%s"' % (p, lport["name"])
            if extra_match_val:
                match += " && %s" % extra_match_val
